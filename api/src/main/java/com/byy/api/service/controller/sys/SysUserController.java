...
import com.byy.api.service.vo.action.CommonVO;
...
public class SysUserController extends CommonController<SysUserService> {
   ...
   */
  @GetMapping("/page")
  public ResponseObject<Map<String, Object>> getSysUser(IPageForm pageForm) {
    IPage<SysUser> page =
        baseService.page(
            pageForm.newPage(), Wrappers.<SysUser>lambdaQuery().orderByDesc(SysUser::getUpdatedAt));
  ...
  @PostMapping
  public ResponseObject<SysUserVO> saveSysUser(@RequestBody SysUserForm form) {
    checkUserNameExists(form.getUsername(), null);
    // 保存用户
  ...
  public ResponseObject<SysUserVO> modifySysUser(@RequestBody SysUserForm form) {
    checkUserNameExists(form.getUsername(), form.getId());
    // 更新用户
   ...
   */
  @DeleteMapping("/{id}")
  public ResponseObject<CommonVO> removeSysUser(@PathVariable Long id) {
    return trueOrError(
  ...
  private void saveSysAuthorities(SysUserForm form) {
    List<String> menus = form.getSysAuthorities();
